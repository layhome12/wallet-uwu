...
class _LoginPageState extends State<LoginPage> {
  ...
  void formSubmitted() async {
    ...
    if (_formKey.currentState!.validate()) {
      try {
        await AuthApi().authLogin(username.text, password.text).then(
              (value) => {
                if (value.statusCode == 200)
                  {
                    Navigator.of(context).pushNamed('/main_layout'),
                    authProvider.createToken(
                        value.statusCode, value.message, value.tokenAccess)
                  }
                else
                  {
